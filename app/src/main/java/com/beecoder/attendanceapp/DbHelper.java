...
class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String FACULTY_TABLE_NAME = "FACULTY_TABLE";
    ...
    private static final String CREATE_FACULTY_TABLE =
            "CREATE TABLE " + FACULTY_TABLE_NAME + "( " +
                    KEY_FACULTY_ID + " INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
                    KEY_FACULTY_NAME + " TEXT NOT NULL, " +
                    KEY_FACULTY_EMAIL + " TEXT NOT NULL, " +
                    KEY_FACULTY_PASSWORD + " TEXT NOT NULL, " +
                    "UNIQUE (" + KEY_FACULTY_ID + "," + KEY_FACULTY_EMAIL + ")" +
                    ");";
    ...
    public FacultyBean validateFaculty(String userEmail,String password)
    {
        SQLiteDatabase db = this.getWritableDatabase();

        String query = "SELECT * FROM faculty_table where faculty_email='"+userEmail+"' and faculty_password='"+password+"'";
        ...
        if(cursor.moveToFirst())
        {
            ...
            facultyBean.setFaculty_email(cursor.getString(2));
            facultyBean.setFaculty_password(cursor.getString(3));
            return facultyBean;
        }
        return null;
    }
   ...
   //............................
    ...
    int getPCountMark(long sid) {
        int p_counts_Mark=0;
        String where = " WHERE _SID="+sid+" AND STATUS='P'" ;
        String sql1 = "SELECT COUNT(_STATUS_ID) FROM STATUS_TABLE"+where ;
        Cursor cursorM = getReadableDatabase().rawQuery(sql1,null);

        if(cursorM.getCount()>0){
            cursorM.moveToFirst();
            p_counts_Mark = cursorM.getInt(0);
        }
        cursorM.close();
        return p_counts_Mark;
    }

    int getACountMark(long sid) {
        int a_counts_mark=0;
        String where = " WHERE _SID="+sid+" AND STATUS='A'" ;
        String sql2 = "SELECT COUNT(_STATUS_ID) FROM STATUS_TABLE"+where ;
        Cursor cursor2 = getReadableDatabase().rawQuery(sql2,null);

        if(cursor2.getCount()>0){
            cursor2.moveToFirst();
            a_counts_mark = cursor2.getInt(0);
        }
        cursor2.close();
        return a_counts_mark;
    }




    int getPCount(long sid,String date) {
        int p_counts=0;
        String where = " WHERE _SID="+sid+" AND STATUS='P' AND STATUS_DATE LIKE '%"+date+"'" ;
        String sql = "SELECT COUNT(_STATUS_ID) FROM STATUS_TABLE"+where ;
        Cursor cursor = getReadableDatabase().rawQuery(sql,null);

        if(cursor.getCount()>0){
            cursor.moveToFirst();
            p_counts = cursor.getInt(0);
        }
        cursor.close();
        return p_counts;
    }
    ...
    int getACount(long sid,String date) {
        int a_counts=0;
        String where = " WHERE _SID="+sid+" AND STATUS='A' AND STATUS_DATE LIKE '%"+date+"'" ;
        String sql = "SELECT COUNT(_STATUS_ID) FROM STATUS_TABLE"+where ;
        Cursor cursor = getReadableDatabase().rawQuery(sql,null);

        if(cursor.getCount()>0){
            cursor.moveToFirst();
            a_counts = cursor.getInt(0);
        }
        cursor.close();
        return a_counts;
    }
