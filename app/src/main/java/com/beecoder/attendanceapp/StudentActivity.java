...
public class StudentActivity extends AppCompatActivity {
    ...
    private boolean onMenuItemClick(MenuItem menuItem) {
        if (menuItem.getItemId() == R.id.add_student) {
            ...
            showCalendar();
        }else if (menuItem.getItemId() == R.id.show_Mark) {
            showMark();
    ...
    private void showMark() {
        long[] idArrayMark = new long[studentItems.size()];
        String[] nameArrayMark = new String[studentItems.size()];
        int[] rollArrayMark = new int[studentItems.size()];

        for (int i = 0; i < idArrayMark.length; i++)
            idArrayMark[i] = studentItems.get(i).getSid();
        for (int i = 0; i < rollArrayMark.length; i++)
            rollArrayMark[i] = studentItems.get(i).getRoll();
        for (int i = 0; i < nameArrayMark.length; i++)
            nameArrayMark[i] = studentItems.get(i).getName();


        Intent intentMark = new Intent(this, MarkActivity.class);
        intentMark.putExtra("idArrayMark",idArrayMark);
        intentMark.putExtra("rollArrayMark",rollArrayMark);
        intentMark.putExtra("nameArrayMark",nameArrayMark);
